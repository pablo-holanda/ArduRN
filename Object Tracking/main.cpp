...
int main(int argc, char* argv[])
{
    ...
    int tty_usb;
    char buffer [33];
    int midY = (FRAME_HEIGHT/2);
    int erro = 10;
    int servoTiltPosition = 90;
    int midYY=0;
    int stepSize=1;
    char c[20];
    sprintf(c, "%d", servoTiltPosition);
    ...
    write(tty_usb, &c, 4); //escreve p na porta serial
	...
	while(1){
        ...
        if(midYY > (midY + erro))
            ...
            if(midYY < (midY - erro))
            {
                if(servoTiltPosition <= 175)servoTiltPosition +=stepSize; //Update the tilt position variable to raise the tilt servo. >= 5
            }
